...
// VERSION TAG 
// ==========================================
const APP_VERSION = "1.1.1 (Study Mode Restored)";
...
export default function App() {
  ...
  if (status === 'loading') return <div className="h-screen flex items-center justify-center"><RefreshCw className="animate-spin text-indigo-600" /></div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 font-sans selection:bg-indigo-100">
      ...
      <main className="max-w-4xl mx-auto p-6">
        {view === 'dashboard' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <header className="mb-8"><h1 className="text-3xl font-black uppercase tracking-tight">Classroom</h1></header>
          ...
          <div className="animate-in fade-in duration-500">
            ...
            <div className="space-y-4">
              {filteredSets.map(set => (
                <div key={set.id} onClick={() => { setActiveSetId(set.id); setCurrentCardIndex(0); setIsFlipped(false); setView(activeTab === 'flashcards' ? 'study' : 'quiz-ready'); }} className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-50 flex justify-between items-center cursor-pointer hover:border-indigo-100 transition-all">
            ...
            <div className="perspective-1000 w-full h-[450px] cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
              <div className={`relative w-full h-full transition-transform duration-500 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
                ...
                <div className="absolute inset-0 bg-white rounded-[4rem] shadow-2xl flex flex-col items-center justify-center p-12 backface-hidden border border-slate-100">
                  ...
                  <h3 className="text-3xl font-black text-center leading-tight uppercase tracking-tighter">{activeSet.items[currentCardIndex]?.term}</h3>
                ...
                <div className="absolute inset-0 bg-indigo-600 rounded-[4rem] shadow-2xl flex flex-col items-center justify-center p-12 backface-hidden rotate-y-180 text-white">
                  <p className="text-xl font-bold text-center leading-relaxed">{activeSet.items[currentCardIndex]?.definition}</p>
                </div>
              </div>
            </div>
        ...
        {view === 'edit' && activeSet && (
          <div className="animate-in fade-in pb-24">
            ...
            <input value={activeSet.title} onChange={e => updateSet(activeSetId, { title: e.target.value })} className="w-full text-4xl font-black bg-transparent outline-none mb-10 uppercase border-b-4 pb-4" />
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-6 flex justify-around items-center z-50">
        <Home onClick={() => setView('dashboard')} className={`cursor-pointer ${view === 'dashboard' ? 'text-indigo-600' : 'text-slate-200'}`} />
        <LayoutGrid onClick={() => { setActiveTab('flashcards'); setView('library'); }} className={`cursor-pointer ${view === 'library' && activeTab === 'flashcards' ? 'text-indigo-600' : 'text-slate-200'}`} />
        <GraduationCap onClick={() => { setActiveTab('quizzes'); setView('library'); }} className={`cursor-pointer ${view === 'library' && activeTab === 'quizzes' ? 'text-indigo-600' : 'text-slate-200'}`} />
        <User onClick={() => setView('profile')} className={`cursor-pointer ${view === 'profile' ? 'text-indigo-600' : 'text-slate-200'}`} />
      </div>
