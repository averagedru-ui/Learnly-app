...
// VERSION TAG 
// ==========================================
const APP_VERSION = "1.1.2 (Score Badges Restored)";
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center font-sans">
      <div className="w-full max-w-md bg-white p-10 rounded-[3.5rem] shadow-2xl border">
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          ...
          <button className="w-full bg-indigo-600 text-white py-5 rounded-3xl font-black uppercase text-xs shadow-xl tracking-widest active:scale-95 transition-all">{authMode === 'login' ? 'Sign In' : 'Sign Up'}</button>
        </form>
        <button onClick={() => setAuthMode(authMode === 'login' ? 'signup' : 'login')} className="mt-8 text-indigo-600 font-black text-[10px] uppercase underline">Switch to {authMode === 'login' ? 'Sign Up' : 'Login'}</button>
        ...
        {view === 'dashboard' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10 text-center">
              ...
              <div className="bg-white p-6 rounded-3xl shadow-sm"><div className="text-2xl font-black text-indigo-600">{stats.avg}%</div><div className="text-[10px] font-black uppercase text-slate-300">Avg Score</div></div>
        ...
        {view === 'study' && activeSet && (
          <div className="animate-in fade-in max-w-lg mx-auto py-10">
            ...
            <div className="flex justify-between mt-12 gap-4">
              <button disabled={currentCardIndex === 0} onClick={() => { setCurrentCardIndex(currentCardIndex - 1); setIsFlipped(false); }} className="flex-1 bg-white p-6 rounded-3xl border shadow-sm font-black uppercase text-xs disabled:opacity-20 flex items-center justify-center gap-2">
                <ChevronLeft size={18}/> Prev
              </button>
              <button disabled={currentCardIndex === activeSet.items.length - 1} onClick={() => { setCurrentCardIndex(currentCardIndex + 1); setIsFlipped(false); }} className="flex-1 bg-indigo-600 text-white p-6 rounded-3xl shadow-xl font-black uppercase text-xs disabled:opacity-20 flex items-center justify-center gap-2">
             ...
             <div className="bg-white p-14 rounded-[4rem] shadow-2xl border relative overflow-hidden">
                ...
                {activeSet.lastScore !== undefined && (
                  <div className="mb-8 p-4 bg-slate-50 rounded-3xl">
                    <p className="text-[10px] font-black uppercase text-slate-300 mb-1">Last Score</p>
                    <p className="text-2xl font-black text-indigo-600">{activeSet.lastScore}%</p>
                  </div>
                )}
              ...
              {activeSet.items?.map((item, i) => (
                <div key={i} className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-50 space-y-4">
                  ...
                  {activeTab === 'flashcards' && (
                     <textarea value={item.definition} onChange={e => { const ni = [...activeSet.items]; ni[i].definition = e.target.value; updateSet(activeSetId, {items:ni}); }} className="w-full bg-slate-50 p-6 rounded-3xl font-bold outline-none h-32 border-2 border-transparent" placeholder="Answer or Definition..." />
                  )}
                  {activeTab === 'quizzes' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {['a','b','c','d'].map(k => (
                  ...
                  <input value={item.imageUrl || ''} placeholder="Imgur Link" onChange={e => { const ni = [...activeSet.items]; ni[i].imageUrl = fixImageUrl(e.target.value); updateSet(activeSetId, {items:ni}); }} className="w-full bg-slate-50 p-4 rounded-2xl text-[10px] font-black outline-none uppercase tracking-widest text-slate-400" />
